...
- **Balances are live.** Ethereum needs an Etherscan key through `apiKey` or `ETHERSCAN_API_KEY`; Bitcoin Cash takes `BLOCKCHAIR_API_KEY` once Blockchair starts refusing. The balance counts the chain's own coin, so the 100 DAI of `arweave/weave9` is not in it. There is no retry or rate limiting, so don't loop the call over hundreds of addresses.
