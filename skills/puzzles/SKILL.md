...
No tools in this harness? The CLI prints the same answers: `puzzles collections`, `puzzles list`, `puzzles show <id> --json`, `puzzles hints <id>`, `puzzles verify <id>`, `puzzles balance <id>`, `puzzles authors [key]`, `puzzles solvers [key]`. Run it without installing through `pnpm dlx @agntn/puzzles <command>`, or expose the tools with `puzzles mcp`.
