...
`ktimesg/80-bit` resolves as `80-bit` or `ktimesg/80-bit`. The collection is named after kTimesG, not after this one challenge. If another one comes, it goes right next to this one.
