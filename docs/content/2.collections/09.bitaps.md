...
The public key is known because the zpub gives it. The private key isn't. A path plus an xpub is no secret, so `hasPrivateKey()` says `false`. The top-ups since 2020 are on the record as `increase` transactions. That's why the prize has change on the end.
