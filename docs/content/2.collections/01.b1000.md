...
description: 256 addresses whose keys are 1 to 256 bits wide. The one every scanner runs against. Keys published up to 135 bits and the rest still funded.
---
...
```ts
b1000.solved().length; // keys published by whoever found them
b1000.unsolved().length; // still funded, still waiting
(await selectPuzzles({ collection: "b1000", status: Status.Swept })).length; // emptied once the public key leaked
```
