...
Every Zden record has `assets({ puzzle: "…png" })` pointing at the image under `assets/zden/` in the repository, so `assetPath()` and `assetUrl()` answer for all fifteen. Thirteen carry the public key, four carry the private key. The rest were solved with the key never posted. They're `solved` on the author's word, with the claim transaction on the record and `verifyPuzzle` answering `unavailable`.

Seven of those solved records also ship the solver image he posted next to the puzzle. Levels 1, 2, 3, SFX, XM17, Nethemba and XIXOIO. The page linked them. The records didn't. `assets({ solver })` is the same field Movie Enigma already used, and `puzzles_show` prints it as `solver asset:`.
