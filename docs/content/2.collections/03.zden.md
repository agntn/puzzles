...
```ts [src/collections/zden/decred-janus.ts]
export const zdenPuzzleDecredJanus = decredPuzzle({
  ...
  assets: assets({ puzzle: "decred-janus/puzzle.svg", hints: ["decred-janus/hint.svg"] }),
  hints: [
    official(
      "33*bbb",
...
The 777 sat tx on `zden/1bitcoin-white-paper` isn't the claim. It pays the notarization address and sends change back. The actual sweep lands four days later. His page says the image's SHA-256 is that wallet's private key, so the record quotes that as `official(…)`. The puzzle wallet key is a different one: JTobcat, Ziot and Motive printed it in their writeup, and the record stores that hex and WIF.

## Status

One puzzle is still open: `zden/level-halv`, the halving puzzle from April 2024. The only thing the author said about it, that it is way easier than level 5, is on the record. Level 5 itself is `claimed`, the story is below. The rest is solved. Want to see what a solved multi chain record looks like? This is the collection.

## Level 5, claimed after eight years

`zden/level-5` was the longest wait in this collection. Zden funded `1cryptoGeCRiTzVgxBQcKFFjSVydN1GW7` on 20 October 2018 and topped it up three times to 0.0055555 BTC. On 12 December 2021 Zden relaunched it with a corrected image and new hints, because the 2018 release was, in Zden's own words, incomplete. The record quotes that statement and the 2018 hint tweet, so `puzzles_hints` on `zden/level-5` says which image the winner had to work from.
...
Looking for the solution? It isn't out there yet. The image, the hints and the relaunch statement are on the [puzzle page](/collections/zden/level-5). Once the winner or Zden publishes the key, the record moves to `solved` with the key and the name.
