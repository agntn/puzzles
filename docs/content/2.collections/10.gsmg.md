...
Open since 2019, and a singleton like Bitaps: the id is `gsmg`. The puzzle image and the rabbit hint are under `assets/gsmg/`: `assetUrl()` points at the image on GitHub, `assetLinks()` at both.
