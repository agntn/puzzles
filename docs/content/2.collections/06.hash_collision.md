...
icon: i-lucide-hash
description: Peter Todd's six P2SH scripts from 2013 that pay whoever presents two different inputs with the same hash. SHA-1 was collected in 2017. Four still stand.
---
...
In September 2013 Peter Todd funded six P2SH addresses whose redeem scripts pay out to anyone with a hash collision. Two different preimages, same value under SHA-1, SHA-256, RIPEMD-160, HASH160 or HASH256, plus one for an `OP_ABS` quirk. It's a bounty on the hash function itself. The prize sits on chain where anyone can top it up, and people did.

The SHA-1 one paid out in 2017 after the SHAttered collision was published. The record dates the claim to the transaction that finally spent it. The other four hash bounties are still funded. Nobody has a SHA-256 collision. And if you have one, there's a faster way to make money than 0.28 BTC.
...
export const hashCollisionPuzzleSha1 = bitcoinPuzzle({
  ...
  transactions: [
    ...
    claim("9ec2…14e…", "2023-02-22 23:00:49", 2.48),
  ],
});
```
