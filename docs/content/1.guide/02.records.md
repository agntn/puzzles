...
A hint that holds for every puzzle in a collection goes on the collection, once, as the fourth constructor argument next to the key, the author and the list: `super(key, author, puzzles, hints)`. A puzzle inherits those and adds its own on top. `collection.hintsFor(query)` and `collection.hintsById(id)` hand back the joined list, the collection's first, and `puzzles_show` prints the inherited ones as `collection hints` above the puzzle's `hints`. The serialized dataset carries them once per collection too.
