...
`dataVersion()` changes when a record changes and stays the same across runtimes. Cache a snapshot, compare it later, no build step. No timestamp in it, no environment data. The snapshot is frozen through, rows and records included, so decorating a serialized record is a `TypeError` in strict mode rather than a rewrite of the export for every other reader. `puzzles export` prints `dataset()`. That's what the JSON consumers of the old dataset read now.
