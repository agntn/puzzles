...
description: A manifest of ten keys and ten lazy imports. Keys and aliases answer at once. Records load on the first lookup for their collection. How to register yours.
---
...
An entry with a loader stays lazy, the same as the built-ins. An instance registers as resolved. The same instance twice is a no-op. Another instance or entry under an existing key replaces what the key held, `b1000` included. A fork with corrected data can stand in for the shipped one. Every aggregate view refreshes on the next read. `all()`, `stats()`, `dataVersion()` and the tools are memoized per registry snapshot, and a registration starts a new one. The whole recipe is in [Custom collections](/guide/custom).
