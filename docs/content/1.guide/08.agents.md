...
title: Agents
icon: i-lucide-bot
description: The same twelve tools over MCP and the Pi and OMP extensions. What the text carries and where the limits live. The one tool that leaves the process.
---

## Twelve tools, three surfaces
...
| `puzzles_authors`     | Every author with its kind, its collections and its puzzle count                                  | none                                                                        |
| `puzzles_author`      | One author's record: aliases, profiles, addresses and the sourced facts                           | `key`, an author key or a collection key                                    |
| `puzzles_solvers`     | Every named solver with its kind, its solves and the collections it also published                | none                                                                        |
| `puzzles_solver`      | One solver's record: every solve with date and prize, profiles and sourced facts                  | `key`, a solver key or a puzzle identifier                                  |
...
| `puzzles_stages`      | The stages of a multi-stage puzzle in the author's order, with pages, files and published answers | `id`                                                                        |
| `puzzles_list`        | Puzzles filtered by address, collection, chain, status and public key availability                | `address`, `collection`, `chain`, `status`, `withPubkey`, `limit`, `offset` |
...
Every tool is annotated read only and idempotent. Eleven are closed world. `puzzles_balance` is open world, because it reaches a block explorer. A host that lets a model call it should know that.
