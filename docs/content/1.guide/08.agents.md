...
title: Agents
icon: i-lucide-bot
description: The same nine tools over MCP and the Pi and OMP extensions. What the text carries and where the limits live. The one tool that leaves the process.
---
...
Every tool is annotated read only and idempotent. Eight are closed world. `puzzles_balance` is open world, because it reaches a block explorer. A host that lets a model call it should know that.
