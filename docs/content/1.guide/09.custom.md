...
- `registerCollection({ key, load })` keeps the collection lazy. `registerCollection(instance)` registers it resolved, and the same instance twice is a no-op. Either way `get`, `all`, `stats`, `dataVersion` and the twelve tools see it on their next call.
