...
A show result links to the puzzle's own page, `/collections/b1000/71`, with the whole record: every key row, every transaction with its explorer link, the images the record ships, the JSON, and the text a model gets. Every collection page lists its puzzles the same way. All 333 have a page.
