...
const result = await verifyPuzzle(await requirePuzzle("b1000/1"));
...
`verifyPuzzle(puzzle)` returns a promise. It reads `secretOf(puzzle.keyData())`, turns the secret into a raw key, derives the address through the `@agntn/keys` wallet for the chain, and compares. `addressesEqual` lowercases Ethereum and compares everything else byte for byte.
...
```ts
const result = await verifyPuzzle(await requirePuzzle("b1000/71"));
