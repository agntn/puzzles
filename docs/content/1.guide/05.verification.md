...
A seed phrase derives as written. The BIP39 checksum isn't enforced, because the seed is PBKDF2 over the words and a puzzle can publish a phrase whose checksum fails. A word outside the list or a wrong word count still fails, and a wrong phrase of listed words fails on the address comparison.
