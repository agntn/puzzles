...
`get(id)` loads the one collection the identifier names. Anything it doesn't know comes back as `undefined`. Want an exception instead? `requirePuzzle(id)` throws `PuzzleNotFoundError`. Identifiers are `collection/name`. The four singletons, `gsmg`, `bitaps`, `mineshop` and `movie_enigma`, are just the key.

Importing the package evaluates no puzzle records. The registry is a manifest of seventeen keys. The aggregate views are asynchronous because they load on first use:

```ts
import { all, stats } from "@agntn/puzzles";

(await all()).length; // 343, every collection loaded once
