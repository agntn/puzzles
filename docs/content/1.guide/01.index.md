...
Importing the package evaluates no puzzle records. The registry is a manifest of sixteen keys. The aggregate views are asynchronous because they load on first use:

```ts
import { all, stats } from "@agntn/puzzles";

(await all()).length; // 342, every collection loaded once
