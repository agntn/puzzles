...
`get(id)` loads the one collection the identifier names. Anything it doesn't know comes back as `undefined`. Want an exception instead? `requirePuzzle(id)` throws `PuzzleNotFoundError`. Identifiers are `collection/name`. The six singletons, `gsmg`, `bitaps`, `mineshop`, `movie_enigma`, `satoshi_birthday_quiz` and `book_quiz`, are just the key.
