...
`get(id)` loads the one collection the identifier names. Anything it doesn't know comes back as `undefined`. Want an exception instead? `requirePuzzle(id)` throws `PuzzleNotFoundError`. Identifiers are `collection/name`. The three singletons, `gsmg`, `bitaps` and `movie_enigma`, are just the key.

Importing the package evaluates no puzzle records. The registry is a manifest of thirteen keys. The aggregate views are asynchronous because they load on first use:

```ts
import { all, stats } from "@agntn/puzzles";

(await all()).length; // 337, every collection loaded once
(await stats()).unsolved; // 94
```
