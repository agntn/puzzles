...
Peter Todd has been around Bitcoin since 2010 and has argued most of its arguments in public: replace by fee, soft forks over hard forks, the Segwit code review, V3 transactions. He created OpenTimestamps, which is still the standard way to timestamp a file on Bitcoin. The [hash collision bounties](/collections/hash_collision) are from September 2013. Each one is a P2SH script that runs two inputs through a hash and pays if the inputs differ and the hashes don't. The SHA-1 one was collected. The rest are still there and still worth something if you have a collision to sell.
...
```ts [src/collections/hash_collision.ts]
