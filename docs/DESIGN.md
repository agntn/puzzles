...
2. **Areas:** [AppHeaderCTA.vue](app/components/AppHeaderCTA.vue), `Docs` (lit on the guide, the collections, the authors and the solvers) and `Playground`, mono uppercase with the sidebar's square node, filled in the accent for the area you are in. The docs sections moved to the tabs, so they aren't repeated here.
