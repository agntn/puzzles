...
The page ([changelog.vue](app/pages/changelog.vue)) opens with the hero zone like the playground: the ID strip `changelog / @agntn/puzzles v…`, the title, one sentence, three readouts (the releases since the first version, the latest with its date in the accent, the entries with how many are breaking) and the feed as a line of the zone with an `RSS` tag. The `log` circuit runs into the release log.

The log ([ChangelogFeed.vue](app/components/content/ChangelogFeed.vue)) is one instrument on the shared shell, its releases on `UChangelogVersions` and `UChangelogVersion`, styled through their `ui` slots rather than a list of our own:

1. **Bar:** `Log` and `CHANGELOG.md`, one tick per release (open in the accent when the release has a breaking entry), the release count and the date range.
2. **Rail:** the component's line in `--console-line` with its scroll beam in the accent; each release is a square node on it, its inner square in the accent for the latest. From lg the date sits left of the rail, day in mono and the npm publish time under it, dimmed; the heading dates of the Keep a Changelog years have no time. Below lg the date moves above the version.
3. **Release:** the version in mono 18 px as the release's anchor link (`#v0-23-0`), `Latest` and `N breaking` as accent tags, then one dimmed mono line counting the entries per group.
4. **Groups:** each group a rule title with its count; entries in Figtree 14 px behind a short dash, the scope as a tag and `Breaking` as an accent tag before the text. Inline code keeps the docs' boxed style and wraps, unlike the landing's short snippets, because the older entries carry whole paths and calls.
5. **Release foot:** a dashed rule, `↗ release` and `↗ compare`, and the contributors on the right.
6. **Footer:** the feed and where the data comes from.

The page and `/changelog.xml` read the same `/api/changelog`, both prerendered, so the worker never parses the file or asks npm at request time.
...
2. **Areas:** [AppHeaderCTA.vue](app/components/AppHeaderCTA.vue), `Docs` (lit on the guide, the collections, the authors and the solvers), `Playground` and `Changelog`, mono uppercase with the sidebar's square node, filled in the accent for the area you are in. The docs sections moved to the tabs, so they aren't repeated here.
...
`::author-list` in [AuthorList.vue](app/components/content/AuthorList.vue), `::solver-list` in [SolverList.vue](app/components/content/SolverList.vue) and `::collection-list` in [CollectionList.vue](app/components/content/CollectionList.vue) index the author, solver and collection pages, and [LandingCollections.vue](app/components/content/LandingCollections.vue) prints the collection roster on the landing. The rows are a `UTable` inside the clipped shell with the corner crosses. Everything except that shell and the ruler's ticks, which stay in `app.css` as `.roster` and `.roster-ruler`, is Tailwind utilities from `app/utils/roster.ts`: `ROSTER_TABLE_UI` for the table's `ui` prop, `ROSTER_CLASS` for the bar, footer and cell parts every roster shares.

1. **Bar:** the call, `authors()`, `solvers()` or `collections()`, and the row count in the meta. The collection roster names its order there too, `largest first` or `by key ascending`.
...
4. **Rows:** icon and name as the page link, the key in a boxed tag, the one line about it, then a dotted leader into the last column. An author row ends in its collection keys and puzzle count, a solver row in the identifiers of its solves. A collection row carries its chain glyphs next to the key, each in a `UTooltip`, and ends in `77 of 256 open` with the open count in the accent, or `open` and `closed` for a single puzzle; a bare `256` and an unsolved count on a second line read as two unlabeled numbers. Collections start largest first, and a wrapped name keeps the row on its first line's baseline.
