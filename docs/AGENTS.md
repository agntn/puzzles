...
├── app/utils/                     # puzzles (collection presentation, tool names), samples (toSample, builders), puzzle-view (the page's data), record (the record literal, readout and ticks the puzzle dossier and the playground share), landing (static fixtures), collections (collection dossier data), authors (author page and index rows), format
