...
describe("registry consistency", () => {
  ...
  it("refreshes every aggregate after registration and replacement", async () => {
    ...
    expect(originalPuzzles).toHaveLength(352);
    expect(originalCollections).toHaveLength(22);
    expect(originalData).toHaveLength(22);
    ...
    expect(await lib.collections()).toHaveLength(23);
    expect(await lib.all()).toHaveLength(353);
    expect(await lib.stats()).not.toBe(addedStats);
    expect(await lib.stats()).toEqual(addedStats);
    expect(originalStats.total).toBe(352);
