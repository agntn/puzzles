...
describe("registry consistency", () => {
  ...
  it("refreshes every aggregate after registration and replacement", async () => {
    ...
    expect(originalPuzzles).toHaveLength(351);
    expect(originalCollections).toHaveLength(21);
    expect(originalData).toHaveLength(21);
    ...
    expect(await lib.collections()).toHaveLength(22);
    expect(await lib.all()).toHaveLength(352);
    expect(await lib.stats()).not.toBe(addedStats);
    expect(await lib.stats()).toEqual(addedStats);
    expect(originalStats.total).toBe(351);
