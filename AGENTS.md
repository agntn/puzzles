...
- `src/core/puzzle.ts` holds the abstract `Puzzle`, the chain bases (`BitcoinPuzzle`, `BitcoinCashPuzzle`, `EthereumPuzzle`, `LitecoinPuzzle`, `DecredPuzzle`, `ArweavePuzzle`, `MoneroPuzzle`), `Status`, `toJSON()`, and the `PuzzleSpec` record with its chain factories (`bitcoinPuzzle` … `moneroPuzzle`).
- `src/core/parts.ts` holds the constructors a puzzle class calls: addresses (`p2pkh`, `p2sh`, `p2wpkh`, `p2wsh`, `standard`), pubkeys, transactions, assets, stages with their `artifact()` entries (a stage artifact is a file the author published, not a generated data artifact), parties, hints (`official`, `community`, `confirmation`), and the chained `Key` builder.
...
- **Solvers join by key:** a named solver carries a kebab-case `key` on every puzzle it solved, with the same name and kind; `solvers()` in `src/core/dataset.ts` joins those records into one entry, scalars from the first record and list items once each, so a fact lives on the puzzle it is about. A solver known only by an address has no key and no entry. A solver key equal to an author key is the same party, and `test/unit/validation.test.ts` checks both.
...
- **Registration is a lazy manifest:** `src/collections/index.ts` lists `{ key, load }` entries and imports no collection module statically; `src/core/registry.ts` seeds its table from that list on first use, so importing the package has no side effects and `sideEffects` names only the CLI entry. `registerCollection()` stays open for instances and for `{ key, load }` entries. A new collection needs a `static readonly key`, a manifest entry, and nothing else: the `pack.entry` glob in `vite.config.ts` picks up its file and `test/unit/library.test.ts` compares the modules on disk with the manifest and loads each entry.
...
- **One registry:** `src/core/registry.ts` is the only registry. Keep the `peter_todd` and `warpwallet` aliases and the snake_case keys of the collections renamed to kebab-case.
- **Identifiers:** IDs stay `collection/name`, both segments kebab-case, like file names, asset directories and docs URLs; singleton IDs are `gsmg`, `bitaps`, `mineshop` and `movie-enigma`. A collection resolves a query only in the spelling its identifier uses, `71`, `"71"` or `"b1000/71"`, never a `Number()` reading such as `"0x47"` or `"7e1"`, and a query of another type is a miss, not a `TypeError`.
...
- **Verification is lazy:** `verify()` is asynchronous and imports `crypto.ts` on its first call, so keys stays out of the package import and of CLI startup; keep every other module free of static imports from `crypto.ts`. citty resolves every subcommand for `--help` and for an unknown command, and `test/eval-packed.ts` checks that `puzzles --help`, the library import and a lookup load no keys, while the first verification does.
- **Tests share module state:** the `test` block of `vite.config.ts` sets `isolate: false`, so a test that mutates the registry works on a fresh module graph (`vi.resetModules()` plus a dynamic import) and resets the graph again when it is done.

## Change routing

- New puzzle: add `src/collections/<key>/<name>.ts` exporting a record built with the right chain factory, then import it and append it to the collection's `puzzles` list. New export names use concise camelCase, preserving the publisher's word boundaries, for example `luckyLurkerVault2`, without a redundant `Puzzle` segment; the file name is the identifier segment.
...
- New solver: give the `party()` on the puzzle a kebab-case `key`, its `kind` when known and at least one sourced `fact`; a second solve repeats the key and adds only its own facts. Add `docs/content/4.solvers/<n>.<key>.md` with `::solver-facts{solver="<key>"}`.
