...
- **Records stay as written:** the factory freezes the spec through with `frozen()` from `parts.ts`, `Key` freezes its data and `Collection` freezes the author, so every accessor hands back immutable data and the views memoized per registry snapshot stay true. New parts are plain objects and arrays, which the freeze walks; a class with private state freezes its own.
