...
Local reading copies of the pages the collection records cite and nobody else keeps: six tweets and one Reddit thread, as of September 22, 2026. The archive keeps the Ballet announcement, Zden's two hints, the kitten image source used by Bitimage, the two announcements the author records cite, and the whole book quiz thread with its comments, because that thread is where the answer key and the prize address were published. Tweets are archived as single posts: no replies, no quote tweets, no other post about the same puzzle.
...
The pages were fetched and rendered through Steel without account cookies; the two 2026-09-22 author entries were rendered from X's public embed in a local headless Chromium, which is why they show Follow buttons and no engagement counts beyond what the embed prints. The book quiz entry is a local headless Chromium render of the Wayback capture itself, not of the live thread, so it carries the archive banner and Reddit's 2023 chrome. Transcripts were checked against both rendered HTML and screenshots. Each PNG is a crop of the displayed post: author, text, attached image when present, timestamp, and engagement row. Surrounding navigation and other posts were excluded. Cropping and PNG conversion do not restore the original attachment bytes. Use the puzzle's existing files for image analysis, not these screenshots.
...
Run `pnpm exec vitest run test/unit/source-archives.test.ts` after adding an entry. The test checks screenshot hashes, that the archive covers the tweet IDs referenced by the serialized collections, and that a non-tweet entry archives a URL the records actually cite. A tweet entry gives its ID and handle and the test derives the rest; anything else carries its own URL and its own capture URL, because no other page's archive address is derivable. Add the new entry to its provenance cases and compare the transcript with the image by eye. Do not accept a login page, a truncated post, or an unloaded attachment as a capture.
